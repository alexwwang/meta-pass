...
int main(void)
{
    ...
    assert(meta_seq_feed(&st, META_SEQ_KEY_DOWN, 9300) == false);
    assert(meta_seq_feed(&st, META_SEQ_KEY_OK_LONG, 9400) == true);

    // 5. 中间按错键 → 序列中断
    meta_seq_reset(&st);
    assert(meta_seq_feed(&st, META_SEQ_KEY_UP, 10000) == false);
    assert(meta_seq_feed(&st, META_SEQ_KEY_UP, 10100) == false);
    assert(meta_seq_feed(&st, META_SEQ_KEY_OK_LONG, 10200) == false); // 期望 DOWN,打乱
    assert(meta_seq_feed(&st, META_SEQ_KEY_DOWN, 10300) == false);
    assert(meta_seq_feed(&st, META_SEQ_KEY_DOWN, 10400) == false);
    assert(meta_seq_feed(&st, META_SEQ_KEY_OK_LONG, 10500) == false); // 前缀已毁,不命中

    // 6. 重叠前缀: "U U U U D D L" 的最后 5 键是合法序列 → 命中
    meta_seq_reset(&st);
    const meta_seq_key_t overlap[] = {
        META_SEQ_KEY_UP, META_SEQ_KEY_UP, META_SEQ_KEY_UP, META_SEQ_KEY_UP,
        META_SEQ_KEY_DOWN, META_SEQ_KEY_DOWN, META_SEQ_KEY_OK_LONG,
    };
    ...
    assert(meta_seq_feed(&st, META_SEQ_KEY_DOWN, 0xFFFFFFF0u + 150u) == false);
    assert(meta_seq_feed(&st, META_SEQ_KEY_OK_LONG, 0xFFFFFFF0u + 200u) == true);
