...
**教训 1:恢复协议不允许依赖无法观测的对端状态。** 块读取失败后,stub 阻塞在
`handle_flash_read` 的 `SLIP_recv` 等下一个 ACK。此前 L1 恢复补发
`ACK = chunkSize(0x8000)`:stub 仅在 `num_acked >= num_sent` 时放弃本次读 —— 已发送
**小于** 0x8000 时它会把剩余数据继续吐完(恰好延长了毒化 sync 的交叠窗口)。
修复:补发 `ACK = 0xFFFFFFFF` —— 数学上恒 ≥ 任何 num_sent → stub 必然立即中止本次
读、发 digest、回命令循环,残余由加宽(800ms)的排空窗口收干净。恢复从"赌时序"变成
固定序列。
