...
> `build/meta-pass_v<版本>.bin` 写入空白设备；对已有身份的设备，只有合并文件
...
固件门禁使用全新的临时构建目录，并从仓库 `sdkconfig.defaults` 生成隔离的 `sdkconfig`。它不会读取或覆盖开发者根目录的 `sdkconfig`，只把验证通过的合并镜像复制到 `build/meta-pass_v<版本>.bin`（版本取自 `git describe`）。门禁同时强制检查[受保护的 Flash 布局](protected-flash-layout.zh_CN.md)：受保护的 `cardid` 地址、应用大小、分区表 MD5 以及单机身份数据不入包。
...
社区只能上传验证通过的 `build/meta-pass_v<版本>.bin`，不得上传应用单镜像
