...
- 项目总览见根 `README.zh_CN.md`；需要发现其它文档时再读取 `docs/README.zh_CN.md`。
- 修改某个事实时，更新其权威来源以及直接引用该事实的文档，不复制新的事实源。

## 文档职责

- **基线文档**（继承自上游 AI Passport 项目）：描述硬件、BSP、工程约束和验收方法，以本仓库真实工具和流程为准保持准确。
- **通用协作与工程规范**：描述代码风格、测试、提交、CI 和 AI 工作流，必须以本仓库真实工具和流程为准。
- **本项目文档**：描述 meta-pass 的产品需求、业务逻辑或素材，放在根 `README` 配对或 `docs/assets/`。

## 放置位置

- 仓库根目录仅保留工具发现入口（`AGENTS.md`、`CLAUDE.md` 及其翻译）、项目 README 配对、许可证、构建清单和 ESP-IDF 配置。
