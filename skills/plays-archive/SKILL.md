...
(not the archive), use the official product references in the upstream repo
([FoloToy/ai-passport `docs/brand/`](https://github.com/FoloToy/ai-passport/tree/main/docs/brand)):
...
as they are, and redraw only the reference's screen region into the play's
actual on-screen content, keeping the screen's size, aspect ratio, corners, and
position identical to the reference.
